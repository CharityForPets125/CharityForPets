...
export default async function ShopPage() {
  const products = await fetchSanity<Product[]>(PRODUCTS_QUERY, {}, []);

  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <h1 className="text-4xl font-bold tracking-tight text-amber-950">Shop</h1>
      ...
      <section className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {products.map((product) => (
          <Link key={product._id} href={`/shop/${product.slug?.current}`} className="group rounded-3xl border border-amber-900/10 bg-white p-4 shadow-sm transition hover:-translate-y-0.5">
            ...
            <h2 className="mt-4 text-xl font-semibold text-amber-950">{product.name}</h2>
