        ...
        <main className="mx-auto w-full max-w-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
            <div className="relative overflow-hidden rounded-2xl border border-emerald-900/10 bg-white shadow-sm sm:rounded-3xl">
                ...
                <div className="relative flex items-center justify-between gap-4 p-5 sm:p-8">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-emerald-950 sm:text-3xl lg:text-4xl">{t("auth.signUp")}</h1>
                        <p className="mt-2 text-sm text-emerald-900/80 sm:mt-3 sm:text-base">{t("auth.createAccount")}</p>
                    </div>
            ...
            <div className="mt-8 rounded-3xl border border-emerald-900/10 bg-white p-6 shadow-sm">
                {query.error && (
                    <p role="alert" className="mb-4 rounded-2xl bg-red-50 px-4 py-3 text-base text-red-700">{query.error}</p>
                )}
                {!supabase ? (
                ...
                <Link href={localizePath("/login", locale)} className="mt-4 inline-block text-base font-semibold text-emerald-700 hover:underline">
                    {t("auth.alreadyHave")}
                </Link>
            </div>
        </main>
